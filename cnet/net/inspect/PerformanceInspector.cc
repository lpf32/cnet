#include <cnet/net/inspect/PerformanceInspector.h>
...
namespace
{
string readProcFile(pid_t pid, const char* basename)
{
    char filename[256];
    snprintf(filename, sizeof filename, "/proc/%d/%s", pid, basename);
    string content;
...
{
    string cmdline = boost::replace_all_copy(readProcFile(getpid(), "cmdline"), string(1, '\0'), "\n\t");
    return cmdline;
}
