...
BOOST_AUTO_TEST_CASE(testBufferShrink)
{
    ...
    buf.retrieve(1500);
    BOOST_CHECK_EQUAL(buf.readableBytes(), 2000-1500);
    BOOST_CHECK_EQUAL(buf.writableBytes(), 0);
...
BOOST_AUTO_TEST_CASE(testBufferFindEOL)
{
    ...
    BOOST_CHECK_EQUAL(buf.findEOL(buf.peek()+9000), null);
}

BOOST_AUTO_TEST_CASE(testBufferFindCRLF)
{
    Buffer buf;
    string str(100, 'x');
    str[0] = '\r';
    ...
    buf.append(str);
    const char* null = NULL;
    BOOST_CHECK_EQUAL(buf.findCRLF(), buf.peek());
    BOOST_CHECK_EQUAL(buf.findCRLF(buf.peek()+90), null);
}
