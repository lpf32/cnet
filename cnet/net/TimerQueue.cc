...
int createTimerfd()
{
    ...
    if (timerfd < 0)
    {
        LOG_SYSFATAL << "Failed in timerd_create";
    }
    return timerfd;
}
...
using namespace cnet::net::detail;
...
TimerId TimerQueue::addTiemr(const TimerCallback &cb,
...
TimerId TimerQueue::addTiemr(TimerCallback &&cb,
                             Timestamp when, double interval)
{
...
std::vector<TimerQueue::Entry> TimerQueue::getExpired(Timestamp now)
{
    ...
    std::make_move_iterator(end), back_inserter(expired));
#else
