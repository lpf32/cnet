...
{
    vector<detail::Transition> transitiions;
    ...
    if (f.valid())
    {
        try {
            ...
            for (int i = 0; i < timecnt; ++i)
            {
                ...
                data->transitiions.push_back(Transition(trans[i], localtime, localIdx));
            }
...
const Localtime* findLocaltime(const TimeZone::Data& data, Transition sentry, Comp comp)
{
    ...
    if (data.transitiions.empty() || comp(sentry, data.transitiions.front()))
    {
    ...
    {
        vector<Transition>::const_iterator transI = lower_bound(data.transitiions.begin(),
                                                            data.transitiions.end(),
                                                            sentry,
                                                            comp);
        if (transI != data.transitiions.end())
        {
            if (!comp.equal(sentry, *transI))
            {
                assert(transI != data.transitiions.begin());
                --transI;
            }
        ...
        {
            // FIXME: use TZ-env
            local = &data.localtimes[data.transitiions.back().localtimeIdx];
        }
    }
    return local;
}
