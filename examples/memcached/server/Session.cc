...
{
    Reader(Tokenizer::iterator& beg, const Tokenizer::iterator& end)
        : first_(beg),
          last_(end)
    {
    }
...
void Session::onMessage(const TcpConnectionPtr &conn, Buffer *buf, Timestamp timestamp)
{
...
bool Session::doUpdate(Tokenizer::iterator &beg, const Tokenizer::iterator& end)
{
    if (command_ == "set")
        policy_ = Item::kSet;
...
bool Session::processRequest(StringPiece request)
{
    ...
    SpaceSeparator seq;
    Tokenizer tok(request.begin(), request.end(), seq);
    ...
    {
#ifdef HAVE_TCMALLOC
        reply("VERSION 0.0.1 cnet with tcmalloc\r\n");
#else
        reply("VERSION 0.01 cnet\r\n");
#endif
    }
...
void Session::doDelete(Tokenizer::iterator &beg, const Tokenizer::iterator& end)
{
