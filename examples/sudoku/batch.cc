...
void runClient(std::istream& in, const InetAddress& serverAddr, int conn)
{
    ...
    boost::ptr_vector<SudokuClient> clinets;
    for (int i = 0; i < conn; i++)
    {
        Fmt f("client-%03d", i+1);
        string name(f.data(), f.length());
        clinets.push_back(new SudokuClient(&loop, serverAddr, input, name, done));
        clinets.back().connect();
    }
