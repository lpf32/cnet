...
class Percentile
{
 public:
  ...
  {
    stat << "recv " << cnet::Fmt("%6zd", latencies.size()) << " in-fly " << infly;
  ...
  const cnet::LogStream::Buffer& report() const
  {
    return stat.buffer();
  }

  void save(const std::vector<int>& latencies, cnet::StringArg name) const
  {
    if (latencies.empty())
      return;
    cnet::FileUtil::AppendFile f(name);
    f.append("# ", 2);
