...
public:
    SudokuClient(EventLoop *loop,
            ...
            : name_(name),
              tcpNoDelay_(nodelay),
              clinet_(loop, serverAddr, name_),
              input_(input),
              count_(0)
    {
        clinet_.setConnectionCallback(boost::bind(&SudokuClient::onConnection, this, _1));
        clinet_.setMessageCallback(boost::bind(&SudokuClient::onMessage, this, _1, _2, _3));
    }
    ...
    void send(int n)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            reqeusts_.append(buf, len);
            sendTime_[count_] = now;
            ++count_;
        }

        conn_->send(&reqeusts_);
    }
